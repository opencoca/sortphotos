...
SortPhotos is free software: you can redistribute it and/or modify
...
exiftool_location = os.path.join(
    os.path.dirname(os.path.realpath(__file__)), "Image-ExifTool", "exiftool"
)
...
def parse_date_exif(date_string):
    ...
    if (
        len(date_entries) == 3
        and date_entries[0] > "0000"
        and "." not in "".join(date_entries)
    ):
...
def get_oldest_timestamp(
    data, additional_groups_to_ignore, additional_tags_to_ignore, print_all_tags=False
):
    ...
    for key in data.keys():
        ...
        if (
            (key not in ignore_tags)
            and (key.split(":")[0] not in ignore_groups)
            and "GPS" not in key
        ):
...
def check_for_early_morning_photos(date, day_begins):
    ...
    if date.hour < day_begins:
        print(
            "moving this photo to the previous day for classification purposes (day_begins="
            + str(day_begins)
            + ")"
        )
        date = date - timedelta(
            hours=date.hour + 1
        )  # push it to the day before for classificiation purposes
    ...
    for idx, data in enumerate(metadata):
        ...
        while True:
            ...
            if (not test and os.path.isfile(dest_file)) or (
                ...
                if remove_duplicates and filecmp.cmp(
                    src_file, dest_compare
...
def main():
    ...
    parser.add_argument(
        "-r", "--recursive", action="store_true", help="search src_dir recursively"
    )
    parser.add_argument(
        "-c", "--copy", action="store_true", help="copy files instead of move"
    )
    parser.add_argument(
        "-s", "--silent", action="store_true", help="don't display parsing details."
    )
    parser.add_argument(
        "-t",
        "--test",
